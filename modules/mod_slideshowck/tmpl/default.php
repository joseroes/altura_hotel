...
<div class="slideshowck<?php echo $params->get('moduleclass_sfx'); ?> camera_wrap <?php echo $params->get('skin'); ?>" id="camera_wrap_<?php echo $module->id; ?>"<?php echo $width; ?>>
	<?php foreach ($items as $item) {
        ...
        ?>
	<div <?php echo $datarel . $datatitle; ?>data-thumb="<?php echo $item->imgthumb; ?>" data-src="<?php echo $item->imgname; ?>" <?php if($item->imglink) echo 'data-link="'.$item->imglink.'" data-target="'.$item->imgtarget.'"'; echo $dataalignment . $datatime; ?>>
		<?php if ($item->imgvideo AND $item->slideselect == 'video') { ?>
                ...
                <?php } 
                    if (($item->imgcaption || $item->article) && (($params->get('lightboxcaption', 'caption') != 'title' || $item->imgtarget != 'lightbox') || !$item->imglink)) {
